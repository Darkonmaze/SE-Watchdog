...
function Search-FatalErrors {
    ...
    # Define the regular expression pattern to search for FATAL log entries
    ...
    foreach ($logDir in $torchLogs) {
        ...
        $logFiles = Get-ChildItem -Path $logDir -Filter "Torch-*.log" -ErrorAction SilentlyContinue | Sort-Object LastWriteTime -Descending

        # Check each log file for FATAL errors
        foreach ($logFile in $logFiles) {
            # Read the content of the log file
            $logContent = Get-Content $logFile.FullName -ErrorAction SilentlyContinue

            # Iterate through log content lines
            for ($i = 0; $i -lt $logContent.Length; $i++) {
                # Check current line for a FATAL error
                if ($logContent[$i] -match $pattern) {
                    # Log the FATAL error line
                    Write-Log "Fatal error found in log file $($logFile.Name): $($logContent[$i])"

                    # Log the next 10 lines after the FATAL error
                    $nextLines = [Math]::Min($logContent.Length, $i + 11) - $i - 1
                    for ($j = 1; $j -le $nextLines; $j++) {
                        Write-Log $($logContent[$i + $j])
                    }

                    # After logging the FATAL line and the next 10 lines, continue to the next file
                    break
                }
            }
        }
...
#If the Controller is enabled through the config file
	if ($nexusEnabled) {
		...
		$nexusControllerProcess = Get-Process | Where-Object { $_.ProcessName -like "*Nexus Controller*" } -ErrorAction SilentlyContinue
